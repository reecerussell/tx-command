...
namespace TxCommand.Mongo.Net5.Tests
{
    public class TransactionProviderTests
    {
        ...
        public async Task CommitAsync_WhereSessionIsActive_Commits()
        {
            ...
            client.VerifyAll();
            session.VerifyAll();
            session.Verify(x => x.CommitTransactionAsync(cancellationToken), Times.Once);
        }

        [Fact]
        public async Task CommitAsync_WhereSessionIsNotActive_Throws()
        {
            var session = new Mock<IClientSessionHandle>();
            session.SetupGet(x => x.IsInTransaction).Returns(false);
            ...
            await Assert.ThrowsAsync<TransactionNotStartedException>(() => provider.CommitAsync(cancellationToken));

            client.VerifyAll();
            session.VerifyAll();
            session.Verify(x => x.CommitTransactionAsync(cancellationToken), Times.Never);
        }
